...
# Time: O(log(n)) (coz of sorting)
# Space: O(n)
# I believe a space O(1) solution
# would require exponential time (i.e backtracking)
...
proc solve(n: string): int =
  var words = newStringTable(modeCaseSensitive)
  for line in n.split("\n"):
    inc result
    for word in line.split(' '):
      ...
      if w in words:
        ...
        break
      words[w] = nil
    words.clear(modeCaseSensitive)

echo $solve("abcde fghij")
echo $solve("abcde xyz ecdab")
echo $solve("a ab abc abd abf abj")
echo $solve("iiii oiii ooii oooi oooo")
echo $solve("oiii ioii iioi iiio")
echo $solve("""bdwdjjo avricm cjbmj ran lmfsom ivsof
