...
# Pretty much for the same reasons as the previous one.
# The correct solution would check the trajectory.
# Simulating until all signs are the same on each particle,
# then comparing trajectory of particles with equal sign (- or +)
...
proc solve(s: string): int =
  ...
  var
    particlesNxt: seq[Particle]
    pos: HashSet[array[3, int]]
    dups: HashSet[array[3, int]]
  for _ in 0 .. 1000:  # tics
    pos = initSet[array[3, int]]()
    dups = initSet[array[3, int]]()
    for i, p in particles:
      var curr = [p.px, p.py, p.pz]
      if curr in pos:
        dups.incl(curr)
      else:
        pos.incl(curr)
    particlesNxt = newSeqOfCap[Particle](particles.len)
    for i, p in particles:
      var curr = [p.px, p.py, p.pz]
      if curr in dups:
        dec result
