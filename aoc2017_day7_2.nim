...
proc calcTotalWeight(n: Node): int =
  n.totalWeight.inc(n.weight)
  for c in n.children:
    n.totalWeight.inc(c.calcTotalWeight())
  result = n.totalWeight

proc findBadWeight(n: Node): int =
  # go deepest first then check bad weights
  # from leaf to root. First node with bad weight wins!
  result = -1
  for c in n.children:
    let bw = findBadWeight(c)
    if bw != -1:
      return bw
  var a, b: tuple[n: Node, count: int]
  for c in n.children:
    if a.n.isNil or c.totalWeight == a.n.totalWeight:
      a.n = c
      inc a.count
    else:
      assert(b.n.isNil or c.totalWeight == b.n.totalWeight)
      b.n = c
      inc b.count
  if a.count > 0 and b.count > 0:
    if b.count == 1:
      swap(a, b)
    return a.n.weight - (a.n.totalWeight - b.n.totalWeight)
...
proc solve(n: string): int =
  ...
  for line in n.split("\n"):
    ...
    let node = nodes[name]
    node.weight = weight.parseInt
    node.children = @[]
    for c in children:
      let nc = nodes[c]
      node.children.add(nc)
      nc.parent = node
  # find root
  var root: Node
  ...
  # populate totalWeights
  discard root.calcTotalWeight()
  # find the thing
  result = root.findBadWeight()
