...
# todo: fixme
# This solution is not actually right
# since it simulates X tics and hopes
# the closest particle will always remain the closest,
# which is obviously not true.
# The problem hints one must
# remove particles once they are off the race,
# meaning they are going away in a more or less
# linear direction and won't curve back to origin.
# So, the real solution would be simulating until every particle
# has the same sign in every property, once that happens
# check which one has the lesser acceleration
# (if two have the same, check velocity and if they
# still have the same, check position)
# Thats the non-math way anyway
...
proc dist(p: Particle): int =
  abs(p.px) + abs(p.py) + abs(p.pz)
...
proc solve(s: string): int =
  ...
  for p in s.split("\n"):
    let
      parts = p.split(", ")
      pos = parts[0][3 ..< parts[0].len-1].split(',')
      vel = parts[1][3 ..< parts[1].len-1].split(',')
      acc = parts[2][3 ..< parts[2].len-1].split(',')
    particles.add((
      px: pos[0].parseInt,
      py: pos[1].parseInt,
      pz: pos[2].parseInt,
  ...
  for _ in 0 .. 1000:  # tics
    for i, p in particles:
      particles[i] = (
        ...
        ay: p.ay,
        az: p.az)
  var minDist = particles[0].dist
  for i, p in particles:
    if p.dist < minDist:
      minDist = p.dist
      result = i

echo solve("""p=<1500,413,-535>, v=<-119,22,36>, a=<-5,-12,3>
